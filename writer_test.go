...
func TestMediaPlaylistSetDefine(t *testing.T) {
	m, err := NewMediaPlaylist(1, 1)
	if err != nil {
		t.Fatalf("Create media playlist failed: %s", err)
	}
	...
	// m.AppendDefine("IMPORT", DefineType_Import)
	// m.AppendDefine("QUERY", DefineType_QueryParam)
}
